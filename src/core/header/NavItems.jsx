...
const NavItems = ({ ToggleMenu, MenuOpen, isActive, }) => {
  ...
  return (
    <>
      <nav className={`${MenuOpen ? 'translate-x-0' : '-translate-y-full'} duration-300 mx-auto  poppins-medium   rounded-b-lg text-white bg-[#262341] min-h-50 max-h-screen max-w-full w-full top-0 right-0 absolute z-50`}>
          ...
          <ul className="flex flex-col items-start mt-0 *:my-2 *:px-2  overflow-y-auto max-h-[calc(100vh-150px)] ">
            ...
            <li ref={dropdown2Ref} >
              <Link onClick={() => setDropdown2(!dropdown2)} className={`flex items-center gap-1  ${author.some(link => location.pathname === link.path)
                ? 'text-[#262341] bg-white  py-1 px-1 rounded-lg'
                : ''
                }`}>
                Author’s Desk
                <FaChevronDown
            ...
            <Link to="/contact-us" className={` ${isActive == '/contact-us' ? 'text-[#262341] bg-white  rounded-lg min-w-[75px] py-1  px-2' : ''}`}>
              <li
                onClick={() => {
                  ToggleMenu(false);
